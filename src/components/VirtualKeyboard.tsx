...
const VirtualKeyboard = ({ onType, value, placeholder }: VirtualKeyboardProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="bg-gradient-to-b from-card/95 via-card to-card/98 backdrop-blur-xl border-t-4 border-primary/40 shadow-[0_-10px_50px_-10px_rgba(168,85,247,0.4)] p-4 max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
        ...
        {/* Display Area */}
        <div className="w-full mx-auto bg-input/50 rounded-lg p-3 mb-4 text-lg font-serif text-left min-h-[52px] break-words border border-border/50">
          {value || <span className="text-muted-foreground">{placeholder}</span>}
        </div>
